package com.spartanullnull.otil.domain.user.exception;

import com.spartanullnull.otil.domain.common.exception.*;
import com.spartanullnull.otil.global.exception.*;
import java.time.*;

public class DuplicatedAccountIdException extends DomainException {

    public DuplicatedAccountIdException(String field, String value, String reason) {
        super(
            ErrorCode.of(ErrorCase.DUPLICATED_ACCOUNT_ID),
            new ErrorDetail(field, value, reason, LocalDateTime.now()
            )
        );
    }

    public DuplicatedAccountIdException(String field, String value) {
        super(
            ErrorCode.of(ErrorCase.DUPLICATED_ACCOUNT_ID),
            new ErrorDetail(field, value, LocalDateTime.now()
            )
        );
    }
}
